...
- **كل أيقونات التطبيق مولَّدة من ملف واحد:** `assets/noor-logo.png`. لا تعدّل
  أي أيقونة أو شاشة بداية يدوياً — بدّل ملف المصدر وشغّل
  `npm i --no-save sharp && npm run build:icons`، فيولّد أيقونات المشغّل بكل
  الكثافات (عادية/دائرية/تكيّفية) وشاشات البداية وأيقونات الويب ولون الخلفية
  التكيّفية معاً. تغيير الشعار **يمسّ الأندرويد**، فيلزمه APK جديد.
